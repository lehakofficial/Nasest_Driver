...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim) // колбек по захвату
{
	if(htim->Instance == TIM17)	{
		if(htim->Channel == HAL_TIM_ACTIVE_CHANNEL_1)	{// RISE/FALL
			//Чтение уровня на порте, для определения полярности пришедшего фронта. Исправить номер порта на маленьком процессоре!!!
			if (GPIOA->IDR & 0x0080)	{
...
/*Переменные для отладки регулятора*/
uint8_t flag_star_one = 0;
uint32_t pwr_ustavka_set = 192000;
uint16_t key_adc_recive = 0;
uint16_t adc_v_recive [2000];
uint16_t adc_i_recive [2000];
uint32_t pwr_adc_recive [2000];
uint16_t duty_recive [2000];
/*Переменные для отладки регулятора конец!!!*/
...
//Режим работы стабилизации напряжения DC/DC			******************************************************************************
			...
			/*Для отладки регулятора !!!!!*/
			if (flag_star_one)	{
				//Начинаем запись значений и единичное воздействие
				adc_v_recive [key_adc_recive] = v_adc_filtr_normal;
				adc_i_recive [key_adc_recive] = i_adc_filtr_normal;
				pwr_adc_recive [key_adc_recive] = pwr_uprav;
				duty_recive [key_adc_recive] = duty_cycle;
				if (key_adc_recive)
					pwr_ustavka = pwr_ustavka_set;
//				if (pwr_ustavka < pwr_ustavka_set)
//					pwr_ustavka += 4000;
				
//				adc_v_recive [key_adc_recive] = adc[0];
//				adc_i_recive [key_adc_recive] = adc[1];
				
				key_adc_recive++;
				if(key_adc_recive >= 2000)	{
					key_adc_recive =  0;
					flag_star_one = 0;
					//pwr_ustavka = 0;
				}
			}
			/*Для отладки регулятора, конец!!!!!*/		
			
		
		/*
			uprav_int32 = (int32_t)(uprav);
		
			uprav_duty_cycle = abs(uprav_int32) + MIN_POWER_PWM;
			
			//Проверка на выход за предельные значения ШИМ
			if (uprav_duty_cycle > MAX_POWER_PWM)
				uprav_duty_cycle = MAX_POWER_PWM;
		
			duty_cycle = uprav_duty_cycle;
*/
		}
	GPIOA->ODR &= ~(0x20);//Для настройки на отладке!!!
}
